...
def display_evaluation_results(evaluation_results: Dict, is_quarter: bool = False):
    ...
    if is_quarter:
        col1, col2a, col2, col3 = st.columns(4)
    else:
    ...
    with col3:
        if is_quarter:
            ...
            acceptance_rate = (round(total_accepted_credits,0) / total_credits * 100) if total_credits > 0 else 0.0
        st.metric("Credit Acceptance Rate", f"{round(acceptance_rate,1)}%")
    ...
    if evaluation_results.get('evaluated_courses'):
        ...
        # Inject 'credits_adj' after 'credits' if is_quarter
        if is_quarter and 'credits_adj' in df.columns:
            insert_index = base_column_order.index('credits') + 1
...
def display_combined_results(data: Dict):
    ...
    try:
        ...
        if data.get("courses"):
            ...
            issues = courses_df[courses_df['notes'] != "✓"]
            if not issues.empty:
